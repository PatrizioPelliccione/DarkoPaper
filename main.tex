...
\item There is not a noticeable difference in the resource configuration for one agent %agent variance in the resources of an agent 
...
We  specify the ensemble  type  used  for  safety  problem resolution on Listing~\ref{lstSafety}. To identify the ensemble, we declare a leader  agent  to  be  the  id  of  the  ensemble - essentially saying that instances of this ensemble type cannot be created without the ensemble being associated with a unique agent instance, which can be seen as a coordinator of the ensemble. \patrizio{please explain better. Moreover in the listing (row 2), is id a variable? Then what's Leader? As I understand Agent is the type} 
